 ...
 *    `color-mix()`. The hero's "why it matters" aside is one:
 *    `color-mix(in oklab, var(--accent) 6%, transparent)`, a translucent tint
 *    whose real backdrop is the page background behind it. axe files it under
 *    `incomplete` rather than measuring it.
 *  - text over a surface the STYLESHEET never names, because JavaScript set it
 *    inline. That is the half that matters on this page. Every `.block-cell`
 *    in the ECB grid and the five-mode comparison takes its
 *    `background-color` from a ten- or fifteen-entry palette in `ecb.ts` /
 *    `compare.ts`, chosen per distinct ciphertext block; the CSS only says
 *    `color: #fff`. Reading the stylesheet tells you nothing about what that
 *    white sits on — you have to encrypt something and measure the frame.
 ...
 *    `.cl-hero-sub { opacity: .85 }` and the `pulse` keyframe that takes
 *    `.oracle-byte.active` to `opacity: .6` mid-cycle. The gate this replaces
 ...
 *    fabricates a pass for exactly this case. Nothing is injected here.
 *
 ...
 *    boxes are every `.hex-output` (`max-height: 6rem` with `overflow-x` and
 *    `overflow-y: auto` around a full ciphertext), the padding oracle's
 *    `.log-scroll` (`max-height: 12rem`, thousands of query lines), the
 *    horizontally scrolling `.mode-selector`, and the `overflow: hidden`
 *    `.comparison-table`. Content scrolled past those boxes is not dimmed or
 *    partly drawn — it is absent from the frame, and asking what colour it
 ...
 *    marks plus the footer's GitHub mark, all `aria-hidden` and glyph-free — so
 *    the sibling walk finds nothing to do. It is kept because the alternative is
 *    a helper that silently mismeasures the first labelled diagram anyone adds:
 ...
 *    report a ratio nothing on screen has. The lab's two `<canvas>` elements are
 *    a different case entirely and one nothing here can measure: pixels drawn
 *    into a canvas are outside CSS, so the ECB penguin comparison is judged by
 *    the labels around it, not by the image inside it.
 */
...
export async function auditContrast(page: Page): Promise<ContrastFailure[]> {
  return page.evaluate((rootSelector) => {
     ...
     * `url("data:image/svg+xml,…")` chevron on every `.input-group select`.
     */
     ...
     *
     * A driven scan here walks a whole mode panel plus the always-present
     * decision tree, threat grids and explainers, and after the padding oracle
     * has run it also walks a sixteen-cell byte grid and a query log with
     * hundreds of entries. Every one of those nodes re-walks the same handful
     * of ancestors up to `<body>`. Without the caches the pass re-reads the
     * same computed styles and rects tens of thousands of times, and the gate
     * scans sixty-odd states per configuration. Nothing mutates the DOM during
     ...
     * that is every `.hex-output`, the oracle's `.log-scroll`, the
     * `.mode-selector` tab strip and the `overflow: hidden` `.comparison-table`.
     */
    ...
    const svgUnderlay = (el: Element, box: DOMRect): RGBA => {
      ...
      // ratio was 6.15:1. This lab draws no SVG shapes of its own, so the guard
      // costs nothing here and stays for the next diagram.
    ...
    const isVisible = (el: Element): boolean => {
      ...
      // paints nothing. `checkVisibility()` catches it, and it matters twice
      // over here: six of the seven `role="tabpanel"` sections carry the
      // `hidden` attribute at any moment, and each comparison card wraps its
      // ciphertext hex in a closed `<details>`. The gate reaches every one of
      // those by clicking the tab or the summary, rather than setting
      // `open = true` on all of them and stripping every `[hidden]` attribute
      // from script — which is what the gate it replaces did, producing a
      // document with all seven panels stacked at once that no visitor is ever
      // shown.
     ...
     * it, the `aria-hidden` decoration this lab carries — the `■` swatches in
     * the ECB block legend, the `→` between CBC chain blocks, and the shared
     * header's and footer's GitHub and hamburger marks — would be measured as
     * if each took its element's declared `color`. Note the boundary cuts both
     * ways and
     ...
     * cheap to make and silent, and this page builds nearly every result region
     * by assigning `innerHTML` from a template literal, which is exactly where
     * it happens: interpolating a string ARRAY into a template literal makes JS
     * join it with commas, leaving a run of "," text nodes behind. They are
